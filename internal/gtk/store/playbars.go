...
func CreatePlaylistsTreeModel(p m3uetcpb.Perspective) (model *gtk.TreeStore, err error) {
	...
	model, err = gtk.TreeStoreNew(QYTreeColumn.getTypes()...)
	if err != nil {
		return
	}
...
// SetPlaybarChanged sets the update-playbar-view function
...
func subscribeToPlaybarStore() {
	...
	appendItem := func(res *m3uetcpb.SubscribeToPlaybarStoreResponse) {
	...
	removeItem := func(res *m3uetcpb.SubscribeToPlaybarStoreResponse) {
		switch res.Item.(type) {
		case *m3uetcpb.SubscribeToPlaybarStoreResponse_OpenPlaylist:
			item := res.GetOpenPlaylist()
	...
	for {
		...
		if res.Event != m3uetcpb.PlaybarEvent_BE_INITIAL &&
			res.Event != m3uetcpb.PlaybarEvent_BE_INITIAL_ITEM &&
			!barTree.receivingOpenItems {
